...
def Cutout(img, v, max_v, **kwarg):
    ...
    xy = (x0, y0, x1, y1)
    # gray
    color = FILL_COLOR_MNIST if (img.size[0]) == 28 else FILL_COLOR
    img = img.copy()
...
def Rotate(img, v, max_v, **kwarg):
    ...
    if random.random() < 0.5:
        v = -v
    return img.rotate(v, RESAMPLE_MODE, fillcolor=FILL_COLOR_MNIST if (img.size[0]) == 28 else FILL_COLOR)
...
def ShearX(img, v, max_v, **kwarg):
    ...
    return img.transform(img.size, PIL.Image.AFFINE, (1, v, 0, 0, 1, 0), RESAMPLE_MODE, fillcolor=FILL_COLOR_MNIST if (img.size[0]) == 28 else FILL_COLOR)
...
def ShearY(img, v, max_v, **kwarg):
    ...
    if random.random() < 0.5:
        v = -v
    return img.transform(img.size, PIL.Image.AFFINE, (1, 0, 0, v, 1, 0), RESAMPLE_MODE, fillcolor=FILL_COLOR_MNIST if (img.size[0]) == 28 else FILL_COLOR)
...
def TranslateX(img, v, max_v, **kwarg):
    ...
    if random.random() < 0.5:
        v = -v
    v = int(v * img.size[0])
    return img.transform(img.size, PIL.Image.AFFINE, (1, 0, v, 0, 1, 0), RESAMPLE_MODE, fillcolor=FILL_COLOR_MNIST if (img.size[0]) == 28 else FILL_COLOR)
...
def TranslateY(img, v, max_v, **kwarg):
    ...
    if random.random() < 0.5:
        v = -v
    v = int(v * img.size[1])
    return img.transform(img.size, PIL.Image.AFFINE, (1, 0, 0, 0, 1, v), RESAMPLE_MODE, fillcolor=FILL_COLOR_MNIST if (img.size[0]) == 28 else FILL_COLOR)
...
class RandAugmentMNIST_mpl(object):
    ...
    def __call__(self, img):
        ...
        img = CutoutConst(img, v=self.m, max_v=28 // 4)
        return img

class RandAugmentCIFAR_mpl(object):
    def __init__(self, n, m, resample_mode=PIL.Image.BICUBIC, fill=(128, 128, 128)):
        global RESAMPLE_MODE, FILL_COLOR
        RESAMPLE_MODE = resample_mode
        FILL_COLOR = fill
